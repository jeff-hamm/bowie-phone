...
The system automatically detects the hostname and loads the appropriate configuration from `config.js`. Each configuration can specify:
...
Edit `config.js` and add your hostname configurations to the `SITE_CONFIGS` object:
...
`config.js` maintains backward compatibility by creating a global `PHONE_CONFIG` object. Existing `app.js` code will continue to work without changes.
